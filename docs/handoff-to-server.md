...
### 완료 보고 — 3-7 콘텐츠·손님 순회·관계 표시

**상태: 완료 · 2026-08-08**

- 예선 빌드는 서버 HTTP를 호출하지 않고 로컬 이식 규칙과 `LocalDayStateStore`를 계속 사용합니다.
- 입장에는 `greet`와 오늘 욕구의 `order`, 반응에는 어긋난 축의 `feedback_high` 또는 `feedback_low`,
  퇴장에는 `leave`를 손님의 말투와 함께 골라 표시합니다.
- 다음 날마다 생성 콘텐츠의 손님 8명을 순서대로 순회합니다. 관계 표시는 친밀도·단계와 현재 공개된
  평소 욕구, 축 범위, 식이 제한만 번들 콘텐츠에서 읽어 보여 줍니다.
- Unity Editor에서 58일차 손님 입장 화면을 확인했습니다. 낯선 손님 친밀도 0에서도 세 번 누적된
  불 세기·간 축 범위가 관찰 공개되는 규칙이 화면에 정확히 나타났습니다.
- Unity EditMode 전체 74/74 통과했습니다.

**상태: 회신함 · 2026-08-08 — 처리 끝**

서버가 `handoff-from-server.md` **3-6절**로 답했다. 물은 넷에 전부 답이 있다.

| 물은 것 | 답이 어디 있나 |
|---|---|
| 관계 상태의 필드·범위·초기값·변경 규칙 | 서버 `docs/server-design.md` **1-1-2절** (원본) |
| 공개 단위와 단계 | 같은 절 |
| 숨은 페르소나를 넘기지 않는 경계 | 같은 절 — **값이 아니라 플래그** |
| 결정적 규칙의 원본과 테스트 | 서버 `RelationshipRulesTest` **25건** |

**API는 오지 않았고, 그것이 답이다.** 예선 빌드는 오프라인이라 서버를 부르지 못한다. 영속화와 API(4-b)는 본선 몫으로 미뤘다.

**만족도는 서버가 다시 계산하지 않는다.** 클라가 계산한 것(만족도·어긋난 축)을 보내고, 서버는 상태 진행(관계 증감·해금)만 정한다.
...
_원래 요청 본문은 지웠다. 합의된 내용이 서버 문서로 옮겨갔고, 이 파일의 머리말이 옮겨간 항목은 여기서 지우라고 정한다._
