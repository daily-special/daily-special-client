...
클라이언트가 화면이나 임시 상태를 먼저 지어 계약을 고정하지 않도록, 아래를
`handoff-from-server.md`와 서버 원본 문서에 알려 달라.

1. 세이브·손님별 관계 상태의 필드, 범위, 초기값과 변경 규칙
2. 공개 가능한 취향 정보의 단위와 단계(축 이상 구간·식이 제약·선호 욕구 등 무엇을 언제
   공개하는지). 번들의 숨은 페르소나 전체를 API로 넘기지 않는 경계를 포함한다.
3. 조회 API와 관계 변화가 발생하는 명령 API의 경로·요청·응답·오류 형태. 요리 제공 후
   만족도 결과를 서버가 다시 계산하는지, 클라이언트 결과를 받는지, 또는 다른 입력으로
   처리하는지도 서버 쪽에서 결정해 달라.
4. 결정적인 생성·판정 규칙이 있다면 원본 위치와 테스트 전체. 클라이언트는 요약본 대신
   그 테스트를 EditMode로 이식한다.

클라이언트는 명세를 받기 전에는 관계 수치·공개 단계·서버 HTTP를 구현하지 않는다. 현재
반응 화면은 로컬 만족도 시연만 한다.
